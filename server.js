...
// In-memory jobs (demo). Production: database.
const jobs = new Map();
...
// We generate smaller images (cheaper/faster) then ffmpeg scales/crops to final size.
...
function guessScenesFromPrompt(prompt, durationSec) {
  ...
  const titles = ["Hook","Value","Features","Proof","CTA","Export","Format","Brand","Launch","Finish"];
...
async function hfGenerateImageToFile({ prompt, width, height, outPath }) {
  ...
  // Retry logic (HF often returns 503 while model loads)
